...
import { useDeliveryNotes, useDeleteDeliveryNote, useConfirmDeliveryNote } from '@web/hooks/use-delivery-notes';
...
export function DeliveryNotesTable() {
  ...
  const query = React.useMemo(() => {
    const q: ListDeliveryNotesQuery = {
      page,
      limit,
      sortBy,
      sortOrder,
  ...
  const handleDelete = React.useCallback(
    async (dn: { id: string; number: string }) => {
      const ok = await confirm({
        ...
        description: (
          <>
            Are you sure you want to delete draft <span className="font-bold">{dn.number}</span>? This action cannot be undone.
          </>
        ),
        confirmText: 'Delete',
        variant: 'destructive',
      ...
      if (ok) {
        toast.promise(deleteMutation.mutateAsync(dn.id), {
          loading: `Deleting DN ${dn.number}...`,
          success: `DN ${dn.number} deleted successfully`,
          error: 'Failed to delete DN',
        });
      }
    },
    ...
    <div>
      <div className="flex flex-wrap items-center gap-2 mb-4 w-full">
        <div className="relative w-full sm:w-64">
          ...
          <Input
            placeholder="Search DN number or SO ref..."
            value={search}
              ...
              <TableRow>
                ...
                >
                  {error instanceof Error
                    ? error.message
                    : 'Something went wrong while fetching delivery notes.'}
                </TableCell>
              </TableRow>
            ) : tableData.length ? (
        ...
        <div className="flex sm:flex-row flex-col justify-between items-center gap-4 mt-4 p-4 border-t bg-card rounded-lg border">
          <div className="flex sm:flex-row flex-col items-center gap-4 w-full sm:w-auto">
            <div className="text-muted-foreground text-sm">
              Showing page {page} of {totalPages} ({total} DN total)
            </div>
          </div>
