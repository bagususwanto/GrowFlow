...
import { useSalesOrders, useDeleteSalesOrder, useConfirmSalesOrder } from '@web/hooks/use-sales-orders';
...
export function SalesOrdersTable() {
  ...
  const handleEdit = React.useCallback(
    (so: { id: string }) => {
      router.push(`/sales/sales-orders/${so.id}/edit`);
    },
    [router],
  );
  ...
  const handleConfirm = React.useCallback(
    async (so: { id: string; number: string }) => {
      const ok = await confirm({
        ...
        description: `Are you sure you want to confirm Sales Order ${so.number}? This will validate stock availability and reserve the items.`,
  ...
  const handleDelete = React.useCallback(
    async (so: { id: string; number: string }) => {
      const ok = await confirm({
        ...
        description: `Are you sure you want to delete SO draft ${so.number}? This action cannot be undone.`,
        confirmText: 'Delete',
        variant: 'destructive',
      });
      if (ok) {
        toast.promise(deleteMutation.mutateAsync(so.id), {
          loading: `Deleting SO ${so.number}...`,
          success: `SO ${so.number} deleted successfully`,
          error: 'Failed to delete SO',
        });
      }
    },
  ...
  const columns = React.useMemo(
    ...
    [handleView, handleEdit, handleConfirm, handleDelete, sortBy, sortOrder, handleSort],
  );
          ...
          <div className="flex sm:flex-row flex-col justify-between items-center gap-4 mt-4 p-4 border-t">
            <div className="flex sm:flex-row flex-col items-center gap-4 w-full sm:w-auto">
              <div className="text-muted-foreground text-sm">
                Showing page {page} of {totalPages} ({total} SO total)
              </div>
            </div>
