...
import { useWarehouses, useDeleteWarehouse } from './use-warehouses';
...
export function WarehousesTable() {
  ...
  const query = React.useMemo(() => {
    ...
    return q;
  }, [page, limit, debouncedSearch, isActive, sortBy, sortOrder]);

  const { data, isLoading, isError, error } = useWarehouses(query);
  ...
  const handleDelete = React.useCallback(
    async (warehouse: { id: string; name: string }) => {
      ...
      if (ok) {
        toast.promise(deleteMutation.mutateAsync(warehouse.id), {
          ...
          <div className="flex sm:flex-row flex-col justify-between items-center gap-4 mt-4 p-4 border-t">
            <div className="flex sm:flex-row flex-col items-center gap-4 w-full sm:w-auto">
              <div className="text-muted-foreground text-sm">
                Showing page {page} of {totalPages} ({total} warehouses total)
              </div>
            </div>
