...
import { useStockMutations } from './use-stock';
import { useWarehouses } from '../warehouses/use-warehouses';
import { getColumns } from './mutation-columns';
...
export function StockMutationsTable() {
  ...
  const warehouseId = urlWarehouseId || 'all';
  ...
  const setWarehouseId = React.useCallback(
    (newWarehouseId: string) => {
      const queryString = createQueryString({ warehouseId: newWarehouseId, page: 1 });
      router.replace(`${pathname}?${queryString}`, { scroll: false });
    },
    [pathname, router, createQueryString]
  );

  const setMutationType = React.useCallback(
    (newType: string) => {
  ...
  const query = React.useMemo(() => {
    const q: ListStockMutationsQuery = {
      page,
      limit,
      sortBy,
      sortOrder,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2 w-full">
        ...
        <Select value={warehouseId} onValueChange={(val) => setWarehouseId(val || 'all')}>
          <SelectTrigger className="w-full sm:w-48 h-9">
            <SelectValue placeholder="Filter by Warehouse">
              {warehouseId === 'all'
                ? 'All Warehouses'
                : warehousesData?.data?.find((w) => w.id === warehouseId)?.name || 'Loading...'}
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
        ...
        <Select value={mutationType} onValueChange={(val) => setMutationType(val || 'all')}>
          <SelectTrigger className="w-full sm:w-48 h-9">
            <SelectValue placeholder="Filter by Type">
              {mutationType === 'all' ? 'All Types' : mutationType}
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
              ...
              <TableRow>
                ...
                >
                  {error instanceof Error
                    ? error.message
                    : 'Something went wrong while fetching stock mutations.'}
                </TableCell>
              </TableRow>
            ) : tableData.length ? (
