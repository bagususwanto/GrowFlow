...
export function StockBalanceTable() {
  ...
  const sortBy = urlSortBy || 'updatedAt';
  ...
  const query = React.useMemo(() => {
    const q: ListStockBalancesQuery = {
      page,
      limit,
      sortBy,
      sortOrder,
    ...
    return q;
  }, [page, limit, debouncedSearch, warehouseId, sortBy, sortOrder]);

  const { data, isLoading, isError, error } = useStockBalances(query);
              ...
              <TableRow>
                ...
                >
                  {error instanceof Error
                    ? error.message
                    : 'Something went wrong while fetching stock balances.'}
                </TableCell>
              </TableRow>
            ) : tableData.length ? (
